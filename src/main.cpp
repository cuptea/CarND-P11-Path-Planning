      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
            ...
            // initialize the following variables for behavior planning.
            bool too_close = false;
            bool turn_left = true;
            bool turn_right = true;

            // check all cars in ego car's current lane and the two neibhoring lanes to decide next behavior. 
            for(int i=0; i<sensor_fusion.size();i++)
            {
                  ...
                  if(d < (2+ 4*lane + 2) and d > (2+ 4*lane - 2))
                  {
                        ...
                        {
                              // spot the ego car is too cloe to the car in front of us in the same lane.
                              too_close = true;
                        } 
                  }
                  // If the checked car in the left lane of the ego car
                  else if(d < (2+ 4*lane - 2) and d > (2+ 4*lane - 6))
                  {
                        ...
                        // it is not safe to change lane only when the following conditions are satisfied:
                        // 1. the check car is in front us and the distance is less then 30 unit.
                        // 2. the check car is behind us, the distance is less then 2 unit and the speed is larger then the ego car's.
                        if(((check_car_s > car_s) and (check_car_s-car_s)< 30) or 
                           ((check_car_s < car_s) and (car_s-check_car_s)< 5 and (car_speed+5)< check_speed))
                        {
                              // due to this car in the left lane, we should not do the lane change to left lane.
                              turn_left = false;                            
                        } 
                  }
                  // If the checked car in the right lane of the ego car
                  else if(d < (2+ 4*lane + 6) and d > (2+ 4*lane + 2))
                  {
            ...
            // if there is a car in front of the ego car in the same lane, the ego car wants to change lane when it is possible. 
            // otherwise the ego char decelerate.
            if (too_close)
            {
                  // 
                  if(turn_left and (lane != 0))
                  {
                        lane -= 1;
                        cout<<"change lane to left lane."<<endl;
                  }
                  else if(turn_right and (lane != 2))
                  {
                        lane += 1;
            ...
            // if it is possible, ego car drives on the middle lane or accelarate until speed reaches the reference speed. 
            else if (ref_vel < 49.5)
            {      
                  
                  if(turn_left and (lane == 2))
                  {
                        lane = 1;
                        cout<<"change lane to middle lane from right most lane."<<endl;
                  }
                  else if(turn_right and (lane == 0))
                  {
                        lane = 1;
                        cout<<"change lane to middle lane from left most lane."<<endl;
                  }
                  else
                  {
                        ref_vel += 0.224; 
            ...
            // fill up the rest of our path panner after filling it with previous points, here we will always output 50 points
